...
;; Portable design facts (no I/O). Mirrors data/rail-tooling-okaimono-design.edn vocation slice.
(def DESIGN-PUBLIC-FACTS
  {:rail-kind RAIL-KIND
   ...
   :cash-usd-micros 0
   :score-surface []
   :note "R1→gated DESIGN only; tooling vocation after care/housing for itonami job-loss"})

(defn design-public-facts
  "Facts-only projection of tooling-okaimono design (no scores; cash≡0; fulfillment never)."
  []
...
(defn load-design
  "Load data/rail-tooling-okaimono-design.edn. Portable under bb and nbb."
  []
  (edn/load-data "rail-tooling-okaimono-design.edn"))

(defn design-edn-invariants
  "Design EDN: cash≡0, vocation after care/housing, fulfillment never live."
  []
  (let [d (load-design)
    ...
    (when-not (zero? (long cash))
      (throw (ex-info "tooling design cash≡0" out)))
