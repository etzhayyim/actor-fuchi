...
(deftest test-seed-noah-hold-machine
  "Seed noah starts held (disclosure fail) — portable seed load."
  (let [seed (edn/load-data "seed-sustenance-graph.kotoba.edn")
        surfs (pp/persons-from-seed seed)
        noah-surf (first (filter #(str/ends-with? (str (:did %)) "noah") surfs))
        rec (first (filter #(str/ends-with? (str (get % ":maintainer/did")) "noah")
