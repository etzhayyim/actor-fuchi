...
(defn- abaki-blocked-ids
  "Read the abaki Anti-Monopoly routing-policy.json (if present) → set of blocked entity ids.
   File I/O only at this edge; absent/malformed → empty set. Portable nbb+bb."
  []
  (try
    (let [f #?(:clj (-> (clojure.java.io/file *file*) .getParentFile .getParentFile .getParentFile
                        (clojure.java.io/file "abaki" "out" "routing-policy.json") str)
               :cljs (let [path (js/require "node:path")
                           fs (js/require "node:fs")
                           p (.resolve path (.dirname path *file*) ".." ".." "abaki" "out"
                                       "routing-policy.json")]
                       (when (.existsSync fs p) p)))
          text (when f
                 #?(:clj (when (.exists (clojure.java.io/file f)) (slurp f))
                    :cljs (when f (.readFileSync (js/require "node:fs") f "utf8"))))]
      (if (and text (seq text))
        (set (map second (re-seq #"\"id\"\s*:\s*\"([^\"]*)\"" text)))
        #{}))
...
(defn provision
  ...
  [rails alloc-id]
  (let [blocked-ids (abaki-blocked-ids)]
    (mapv
     (fn [r]
