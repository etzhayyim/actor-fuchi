...
(deftest test-housing-routes-council
  (let [ev (couple/make-displacement-event
    ...
    ;; L0 held-stress embed carried onto gov row (itonami job-loss path)
    (is (map? (:held-stress r)))
    (is (true? (:held-stress-ladder-refused r)))
    (is (= "held" (get-in r [:held-stress :disclosure-state])))
    (is (false? (get-in r [:held-stress :live])))
    (is (zero? (get-in r [:held-stress :cash-usd-micros])))
    (pp/assert-no-public-scores!
     (dissoc r :gov-package :rail-flow :r2-by-rail :flowable-booking
             :held-stress :membrane-summary :gov-route-hold))
    (pp/assert-no-public-scores! (dissoc (:held-stress r) :note :priority-stack))))
...
(deftest test-package-batch
  (let [ev (couple/make-displacement-event
    ...
    (is (pos? (:gov-held-stress-subjects gp)))
    (is (pos? (:gov-held-stress-ladder-refused gp)))
...
(deftest test-package-batch-tenure-subjects
  (let [ev (couple/make-displacement-event
    ...
    (let [ts (first (:tenure-subjects gp))]
      ...
      ;; gated-receive DESIGN on residual (default refuse; loan never invoked)
      (is (= :refused (get-in ts [:liquidity-gated-receive-status :phase])))
      (is (false? (get-in ts [:liquidity-gated-receive-status :admissible])))
      (is (false? (get-in ts [:liquidity-gated-receive-status :loan-invoked])))
      (is (false? (:liquidity-loan-executed ts)))
      (is (zero? (:liquidity-cash-usd-micros ts))))
    (is (false? (:live out)))
    (is (= 0 (:cash-usd-micros out)))))
