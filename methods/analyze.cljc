(ns fuchi.methods.analyze
  ...
  House style: ':…' strings stay strings; pure fns; file I/O at portable nbb/bb edges;
  the give-only gate (equity/debt/ROI/exit unrepresentable, cash≡0) is ported 1:1 from the
  sibling modules. Portable .cljc (ADR-2607173000)."
...
(defn- out-dir
  []
  #?(:clj (str (java.io.File. (edn/actor-dir) "out"))
     :cljs (let [path (js/require "node:path")]
             (.join path (edn/actor-dir) "out"))))

(defn- ensure-dir! [d]
  #?(:clj (.mkdirs (java.io.File. (str d)))
...
(defn- join-out [name]
  #?(:clj (str (java.io.File. (out-dir) name))
     :cljs (let [path (js/require "node:path")]
             (.join path (out-dir) name))))

(defn -main
  "CLI entry: run analyze over the seed, write report + derived EDN.
  ...
  (let [seed (edn/load-edn (seed-path))
        ...
        outdir (out-dir)]
    (ensure-dir! outdir)
    (write-text! (join-out "allocation-dryrun.md") (report res))
    (write-text! (join-out "allocations.kotoba.edn")
                 (str "{:fuchi/derived\n [\n" (emit (:derived res)) "\n ]}\n"))
