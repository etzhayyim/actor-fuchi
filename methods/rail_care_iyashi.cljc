(ns fuchi.methods.rail-care-iyashi
  ...
  live=false; cash≡0; disclosure held → refuse. Portable .cljc (design EDN under bb + nbb)."
...
;; Portable design facts (no I/O). Mirrors data/rail-care-iyashi-design.edn multi-gen slice.
(def DESIGN-PUBLIC-FACTS
  {:rail-kind RAIL-KIND
   :provider-did PROVIDER-DID
   :priority-stack PRIORITY-STACK
   :multi-gen-first true
   :care-first-order-rank 1
   :care-first-before-rails []
   :care-first-api-path "care-housing-first-path"
   ...
   :cash-usd-micros 0
   :score-surface []
   :note "R1→gated DESIGN only; care is multi-gen substrate #1 for 孫/子"})

(defn design-public-facts
  "Facts-only projection of care-iyashi single-rail design (no scores; cash≡0)."
  []
...
(defn load-design
  "Load data/rail-care-iyashi-design.edn. Portable under bb and nbb."
  []
  (edn/load-data "rail-care-iyashi-design.edn"))

(defn design-edn-invariants
  "Design EDN: cash≡0, multi-gen first (rank 1), live-produce never."
  []
  (let [d (load-design)
    ...
    (when-not (zero? (long cash))
      (throw (ex-info "care design cash≡0" out)))
    (pp/assert-no-public-scores! (dissoc out :priority-stack))
    out))
    ...
    (let [g (or gate (live-gate/make-live-gate {:leg "provision"}))
      ...
      (try
        (let [plan (gated-live-plan r1-pkg g :env e :hold-machine hold-machine)
          ...
          (pp/assert-no-public-scores! (dissoc out :multi-gen-facts))
          out)
