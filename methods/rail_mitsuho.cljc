...
(def MULTI-GEN-FACTS
  ["staple-kcal-floor-supports-caregiver-and-child-households"
   "food-after-care-housing-for-mago-ko-substrate"
   "not-a-happiness-score"
   "imputed-usd-is-accounting-fact-only"])

;; Portable design facts (no I/O). Mirrors data/rail-mitsuho-design.edn care-first slice.
(def DESIGN-PUBLIC-FACTS
  {:rail-kind RAIL-KIND
   ...
   :cash-usd-micros 0
   :score-surface []
   :note "R1→gated DESIGN only; food after care/housing for 孫/子"})

(defn design-public-facts
  "Facts-only projection of mitsuho single-rail design (no scores; cash≡0)."
  []
...
(defn load-design
  "Load data/rail-mitsuho-design.edn (string-keyed). Portable under bb and nbb."
  []
  (edn/load-data "rail-mitsuho-design.edn"))

(defn design-edn-invariants
  "Facts-only checks against design EDN SSoT: cash≡0, live-produce never, score empty.
   care-first-before-rails / api-path read from :rail/care-first-order (孫/子 order)."
  []
  (let [d (load-design)
        ...
        api (or (get cfo ":api-path") (get cfo :api-path) "care-first-mitsuho-path")
