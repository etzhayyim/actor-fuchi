...
(deftest test-design-public-facts-residual
  (let [d (w/design-public-facts)]
    (is (= "liquidity-warifu" (:rail-kind d)))
    (is (= "liquidity-residual-path" (:care-first-api-path d)))
    (is (= ["care" "housing"] (:care-first-before-rails d)))
    (is (true? (:residual-rail d)))
    (is (true? (:member-principal d)))
    (is (false? (:loan-executed d)))
    (is (false? (:live-produce d)))
    (is (false? (:live d)))
    (is (zero? (:cash-usd-micros d)))
    (is (= [] (:score-surface d)))
    (is (some #{"loan-not-invoked"} (:multi-gen-facts d)))))
...
(deftest test-gated-plan
  ...
  (let [pkg (w/r1-dry-package {:alloc-id "a" :imputed-usd-micros-yr 1 :person (person fresh)})
    ...
    (is (= "liquidity-residual-path" (:care-first-api-path st)))
    (is (true? (:residual-rail st)))
