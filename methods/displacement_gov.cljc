...
(defn package-subject
  ...
  (let [r (route-subject subject)
        ...
        ;; Preserve L0/tenure priority-(2) held-stress facts on gov row (no scores).
        hs-carry (select-keys subject
                              [:held-stress :held-stress-ladder-refused
                               :held-stress-all-inkind-refused :l0-held-stress-carried
                               :membrane-summary])
        out (merge r hold hs-carry
                   {:gov-package pkg
    ...
    (pp/assert-no-public-scores!
     (dissoc out :gov-package :rail-flow :r2-by-rail :flowable-booking :gov-route-hold
             :held-stress :membrane-summary))
    (when-let [hs (:held-stress out)]
...
(defn attach-substrate-gated-status
  "All in-kind rails R1→gated-live DESIGN + liquidity residual (member-principal).
   ...
   Liquidity residual also gets gated-receive DESIGN (default refuse; loan never invoked).
   When subject has L0 membrane-summary, refresh liquidity/all-seven facts for residual package.
  ...
  (let [hold (:disclosure-hold subject)
        ...
        liq-st (when (and liq (not= :refused (:phase liq)))
                 (liquidity/gated-live-status liq :hold-machine hold))
        liq-recv (when (and liq (not= :refused (:phase liq)))
                   (wrecv/gated-receive-status liq))
        liq-recv-full-chain?
        (boolean (and (some? liq-st) (some? liq-recv)
        ...
        out (cond-> subject
              ...
              liq (assoc :liquidity-package liq
                         :liquidity-residual-for-housing-hold (boolean council-held?)
                         :liquidity-gated-live-status liq-st
                         :liquidity-gated-receive-status liq-recv
                         :liquidity-loan-executed false
                         :liquidity-cash-usd-micros 0
                         :cash-usd-micros 0))
        ;; Keep L0 membrane-summary liquidity/all-seven in sync when residual is present at G7
        out (if (and liq (:membrane-summary out))
              (let [ms (:membrane-summary out)
                    all-inkind? (boolean (:all-inkind-produce-rails-full-chain-refused ms))
                    ms' (assoc ms
                               :liquidity-gated-receive-admissible
                               (boolean (:admissible liq-recv))
                               :liquidity-loan-executed false
                               :liquidity-member-principal
                               (boolean (:member-principal liq true))
                               :liquidity-cash-usd-micros 0
                               :liquidity-receive-full-chain-refused liq-recv-full-chain?
                               :all-seven-rails-receive-membrane-refused
                               (boolean (and all-inkind? liq-recv-full-chain?)))]
                (pp/assert-no-public-scores! (dissoc ms' :note))
                (assoc out :membrane-summary ms'))
              out)]
    (doseq [st [food-st energy-st care-st hous-st tool-st comp-st liq-st liq-recv]]
    ...
    (when liq
      ...
      (pp/assert-no-public-scores! liq))
    out))
...
(defn- package-subject-list
  ...
  (if-not (seq subjects)
    ...
    (let [govs (mapv #(package-subject-row % apply-ratify?) subjects)
          ...
          hs-n (count (filter :held-stress govs))
          hs-ladder-n (count (filter :held-stress-ladder-refused govs))
      ...
      {:subjects subjects'
       ...
       :gov-held-stress-subjects hs-n
       :gov-held-stress-ladder-refused hs-ladder-n})))
  ...
  (if-not (= :offline-enrolled (:phase pkg))
    ...
    (let [ear (earmark-from-pkg pkg)
          ...
          out (cond-> (assoc pkg
                             ...
                             (or (:gov-held-stress-subjects l4) 0)
                             :gov-held-stress-ladder-refused
                             (or (:gov-held-stress-ladder-refused l4) 0)
                             :gov-live false
                             :live false
                             :cash-usd-micros 0
                             :score-surface []
                ...
                (assoc :tenure-subjects (:subjects ten)
                       ...
                       (or (:gov-held-stress-subjects ten) 0)
                       :tenure-gov-held-stress-ladder-refused
                       (or (:gov-held-stress-ladder-refused ten) 0)))]
      out)))
...
(defn package-batch
  ...
  (if (:gov-packaged? batch)
    ...
    (let [pkgs (mapv #(apply package-cohort % opts) (:packages batch))
          ...
          gov-hs (reduce + 0 (map #(or (:gov-held-stress-subjects %) 0) pkgs))
          gov-hs-ladder (reduce + 0 (map #(or (:gov-held-stress-ladder-refused %) 0) pkgs))
          ten-gov-hs (reduce + 0 (map #(or (:tenure-gov-held-stress-subjects %) 0) pkgs))
          ten-gov-hs-ladder (reduce + 0 (map #(or (:tenure-gov-held-stress-ladder-refused %) 0) pkgs))]
      (assoc batch
             :packages pkgs
             ...
             :gov-held-stress-subjects gov-hs
             :gov-held-stress-ladder-refused gov-hs-ladder
             :tenure-gov-held-stress-subjects ten-gov-hs
             :tenure-gov-held-stress-ladder-refused ten-gov-hs-ladder
             :gov-packaged? true
             :live false
             :cash-usd-micros 0
             :score-surface []
