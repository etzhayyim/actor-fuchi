(ns fuchi.methods.rail-housing-commons
  ...
  disclosure held → refuse. Portable .cljc (design EDN under bb + nbb)."
...
;; Portable design facts (no I/O). Mirrors data/rail-housing-commons-design.edn multi-gen slice.
(def DESIGN-PUBLIC-FACTS
  {:rail-kind RAIL-KIND
   :provider-did PROVIDER-DID
   :priority-stack PRIORITY-STACK
   :multi-gen-first true
   :care-first-order-rank 2
   ...
   :cash-usd-micros 0
   :score-surface []
   :note "R1→gated DESIGN only; housing after care for 孫/子; land-grant never"})

(defn design-public-facts
  "Facts-only projection of housing-commons single-rail design (no scores; cash≡0)."
  []
...
(defn load-design
  "Load data/rail-housing-commons-design.edn. Portable under bb and nbb."
  []
  (edn/load-data "rail-housing-commons-design.edn"))

(defn design-edn-invariants
  "Design EDN: cash≡0, care-first rank 2 after care, land-grant never live."
  []
  (let [d (load-design)
    ...
    (when-not (zero? (long cash))
      (throw (ex-info "housing design cash≡0" out)))
