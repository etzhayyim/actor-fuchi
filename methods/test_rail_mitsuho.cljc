...
(deftest test-design-public-facts-care-first
  "Priority (3) design SSoT: food after care/housing for 孫/子; produce never live."
  (let [d (m/design-public-facts)]
    (is (= "food-mitsuho" (:rail-kind d)))
    (is (= "care-first-mitsuho-path" (:care-first-api-path d)))
    (is (= ["care" "housing"] (:care-first-before-rails d)))
    (is (false? (:live-produce d)))
    (is (false? (:produce-executed d)))
    (is (false? (:live d)))
    (is (zero? (:cash-usd-micros d)))
    (is (= [] (:score-surface d)))
    (is (some #{"food-after-care-housing-for-mago-ko-substrate"} (:multi-gen-facts d)))
    (is (= pp/PRIORITY-STACK (:priority-stack d)))))

(deftest test-gated-status-embeds-care-first-facts
  (let [pkg (m/r1-dry-package {:alloc-id "a" :imputed-usd-micros-yr 1
                               :person (person fresh-d)})
        st (m/gated-live-status pkg)]
    (is (= "care-first-mitsuho-path" (:care-first-api-path st)))
    (is (= ["care" "housing"] (:care-first-before-rails st)))
    (is (some #{"food-after-care-housing-for-mago-ko-substrate"} (:multi-gen-facts st)))
    (is (false? (:produce-executed st)))
    (is (false? (:live st)))
    (is (zero? (:cash-usd-micros st)))))

(deftest test-design-edn-invariants-cash-zero-live-refuse
  "Priority (3) design EDN SSoT: cash≡0, gated live=false, no scores.
   care-first-order read from data/rail-mitsuho-design.edn (孫/子)."
  (let [inv (m/design-edn-invariants)]
    (is (= "food-mitsuho" (:rail-kind inv)))
    ...
    (is (= "care-first-mitsuho-path" (:care-first-api-path inv)))))
