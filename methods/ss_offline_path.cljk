(ns fuchi.methods.ss-offline-path
  ...
  Priority stack path (offline only):
    ...
    (3) rails R1 → gated-live DESIGN (all-seven design-public-facts on summary:
        care→housing→food→energy→tooling→compute→liquidity; default refuse;
        live-produce never)
...
(defn run-food-path
  ...
  (let [enrolled (l0/enroll {:subject-did subject-did
        ...
        ;; Expand rails after L0 scaffold; seed hold from enroll (priority 1+2 reuse)
        person {:did subject-did
                ...
                :cash-usd-micros 0}
        hold0 (or (:disclosure-hold enrolled) (dh/initial person))
        ;; priority (2): continuity tick on rail-expanded person (L0 hold as prior)
        cont-open (disc/tick hold0 person :disclosure FRESH-DISC :reason "ss-path-after-l0-enroll")
        ...
        ;; tooling-okaimono gated-receive + gated-produce (vocation; default refuse)
        tooling-recv-gated (when (and tooling-pkg (not= :refused (:phase tooling-pkg)))
                             (trecv/gated-receive-status tooling-pkg))
        tooling-prod-gated (when (and tooling-pkg (not= :refused (:phase tooling-pkg)))
                             (tprod/gated-produce-status tooling-pkg))
        ...
        ;; compute-murakumo gated-receive + gated-produce (learning/vocation; default refuse)
        compute-recv-gated (when (and compute-pkg (not= :refused (:phase compute-pkg)))
                             (mrecv/gated-receive-status compute-pkg))
        compute-prod-gated (when (and compute-pkg (not= :refused (:phase compute-pkg)))
                             (cmpprod/gated-produce-status compute-pkg))
        liquidity-ack (when (and liquidity-pkg (not= :refused (:phase liquidity-pkg)))
                        (wrecv/receive-from-r1-package liquidity-pkg))
        ;; liquidity-warifu gated-receive (member-principal residual; default refuse; no produce plan)
        liquidity-recv-gated (when (and liquidity-pkg (not= :refused (:phase liquidity-pkg)))
                               (wrecv/gated-receive-status liquidity-pkg))
        ...
        summary {:l0-stage (or (get-in enrolled [:public-person :stage])
                 ...
                 ;; L0 enroll scaffold facts (priorities 1+2) — reused hold machine above
                 :l0-disclosure-state
                 (when-let [s (:disclosure-state enrolled)] (name s))
                 :l0-disclosure-held (boolean (:disclosure-held? enrolled))
                 :l0-entitlements-may-flow
                 (boolean (:entitlements-may-flow? enrolled true))
                 :l0-path (or (:path enrolled) "l0-enroll-offline")
                 ...
                 :mitsuho-design (food/design-public-facts)
                 :mitsuho-live-produce false
                 :mitsuho-care-first-api-path "care-first-mitsuho-path"
                 :mitsuho-care-first-before-rails ["care" "housing"]
                 ...
                 :hikari-design (energy/design-public-facts)
                 :hikari-live-produce false
                 :hikari-care-first-api-path "care-first-hikari-path"
                 :hikari-care-first-before-rails ["care" "housing"]
                 ...
                 ;; priority (3) multi-gen #2 DESIGN embed (housing after care)
                 :housing-design (housing/design-public-facts)
                 :housing-live-produce false
                 :housing-care-first-api-path "care-housing-first-path"
                 :housing-care-first-before-rails ["care"]
                 :housing-care-first-order-rank 2
                 ...
                 ;; priority (3) vocation tooling DESIGN embed (after multi-gen floors)
                 :tooling-design (tooling/design-public-facts)
                 ...
                 (boolean (:admissible tooling-recv-gated))
                 :tooling-gated-receive-phase
                 (when tooling-recv-gated (name (:phase tooling-recv-gated)))
                 :tooling-fulfillment-invoked-on-receive false
                 :tooling-gated-produce-admissible
                 (boolean (:admissible tooling-prod-gated))
                 :tooling-gated-produce-phase
                 (when tooling-prod-gated (name (:phase tooling-prod-gated)))
                 :tooling-fulfillment-executed-on-gated false
                 :tooling-full-chain-refused
                 (and (some? tooling-gated)
                      (some? tooling-recv-gated)
                      (some? tooling-prod-gated)
                      (not (true? (:admissible tooling-gated)))
                      (not (true? (:admissible tooling-recv-gated)))
                      (not (true? (:admissible tooling-prod-gated))))
                 :compute-r1-phase (when compute-pkg (name (:phase compute-pkg)))
                 :compute-gated-admissible (boolean (:admissible compute-gated))
                 ;; priority (3) vocation compute DESIGN embed (itonami job-loss recovery)
                 :compute-design (compute/design-public-facts)
                 ...
                 (boolean (:admissible compute-recv-gated))
                 :compute-gated-receive-phase
                 (when compute-recv-gated (name (:phase compute-recv-gated)))
                 :compute-quota-invoked-on-receive false
                 :compute-gated-produce-admissible
                 (boolean (:admissible compute-prod-gated))
                 :compute-gated-produce-phase
                 (when compute-prod-gated (name (:phase compute-prod-gated)))
                 :compute-quota-executed-on-gated false
                 :compute-full-chain-refused
                 (and (some? compute-gated)
                      (some? compute-recv-gated)
                      (some? compute-prod-gated)
                      (not (true? (:admissible compute-gated)))
                      (not (true? (:admissible compute-recv-gated)))
                      (not (true? (:admissible compute-prod-gated))))
                 :tooling-compute-receive-both-refused
                 (and (some? tooling-recv-gated)
                      (some? compute-recv-gated)
                      (not (true? (:admissible tooling-recv-gated)))
                      (not (true? (:admissible compute-recv-gated))))
                 :tooling-compute-produce-both-refused
                 (and (some? tooling-prod-gated)
                      (some? compute-prod-gated)
                      (not (true? (:admissible tooling-prod-gated)))
                      (not (true? (:admissible compute-prod-gated))))
                 :tooling-compute-full-chain-refused
                 (and (some? tooling-gated) (some? compute-gated)
                      (some? tooling-recv-gated) (some? compute-recv-gated)
                      (some? tooling-prod-gated) (some? compute-prod-gated)
                      (not (true? (:admissible tooling-gated)))
                      (not (true? (:admissible compute-gated)))
                      (not (true? (:admissible tooling-recv-gated)))
                      (not (true? (:admissible compute-recv-gated)))
                 ...
                 ;; care+housing+food+energy+tooling+compute (not liquidity; no produce plan)
                 (and (some? care-gated) (some? housing-gated)
                      (some? food-gated) (some? energy-gated)
                      (some? tooling-gated) (some? compute-gated)
                      (some? care-recv-gated) (some? housing-recv-gated)
                      (some? food-recv-gated) (some? energy-recv-gated)
                      (some? tooling-recv-gated) (some? compute-recv-gated)
                      (some? care-prod-gated) (some? housing-prod-gated)
                 ...
                 ;; priority (3) residual liquidity DESIGN embed (member-principal; loan never)
                 :liquidity-design (liquidity/design-public-facts)
                 :liquidity-live-produce false
                 :liquidity-care-first-api-path "liquidity-residual-path"
                 :liquidity-gated-receive-admissible
                 (boolean (:admissible liquidity-recv-gated))
                 :liquidity-gated-receive-phase
                 (when liquidity-recv-gated (name (:phase liquidity-recv-gated)))
                 :liquidity-loan-invoked-on-receive false
                 :liquidity-loan-executed false
                 :liquidity-member-principal (boolean (:member-principal liquidity-pkg true))
                 :liquidity-cash-usd-micros 0
                 :liquidity-receive-full-chain-refused
                 ;; plan gated + receive refuse (no produce plan for warifu residual)
                 (and (some? liquidity-gated)
                      (some? liquidity-recv-gated)
                      (not (true? (:admissible liquidity-gated)))
                      (not (true? (:admissible liquidity-recv-gated))))
                 ;; all-seven design-public-facts on SS summary (care→…→liquidity order)
                 ...
                 :score-surface []
                 :priority-stack PRIORITY-STACK}
        ;; Embed priority_stack SSoT (1)(2)(3) facts — same invariants as scorecard
        ps-offline
        (try
          (let [ps-opts (merge {:subject-did (or (:subject-did opts)
                                                 "did:web:etzhayyim.com:member:ss-priority-stack")
                                :vow-text (or (:vow-text opts)
                                              "悔い改め・バプテスマ・得度 — ss offline priority stack")
                                :member-signature (or (:member-signature opts)
                                                      "sig-ss-offline-priority-stack")
                                :food-imputed-usd-micros-yr
                                (or (:food-imputed-usd-micros-yr opts) 2000000000)
                                :energy-imputed-usd-micros-yr
                                (or (:energy-imputed-usd-micros-yr opts) 1500000000)
                                :care-imputed-usd-micros-yr
                                (or (:care-imputed-usd-micros-yr opts) 1000000000)
                                :housing-imputed-usd-micros-yr
                                (or (:housing-imputed-usd-micros-yr opts) 12000000000)}
                               (select-keys opts [:subject-did :vow-text :member-signature
                                                  :food-imputed-usd-micros-yr
                                                  :energy-imputed-usd-micros-yr
                                                  :care-imputed-usd-micros-yr
                                                  :housing-imputed-usd-micros-yr]))
                facts (pstack/public-facts (pstack/run-offline ps-opts))]
            (pstack/assert-public-facts! facts)
            facts)
        ...
        summary (assoc summary
                       ...
                       (boolean (and (:all-inkind-produce-rails-full-chain-refused summary)
                                     (:liquidity-receive-full-chain-refused summary)))
                       :priority-stack-offline ps-offline
                       :priority-stack-offline-ok (boolean (:ok ps-offline)))
        out {:path "ss-offline-inkind-rails"
             ...
             :tooling-gated-receive-status tooling-recv-gated
             :tooling-gated-produce-status tooling-prod-gated
             ...
             :compute-gated-receive-status compute-recv-gated
             :compute-gated-produce-status compute-prod-gated
             ...
             :liquidity-gated-receive-status liquidity-recv-gated
    ...
    (pp/assert-no-public-scores! (dissoc summary :rail-gated :continuity-action
                                         ...
                                         :care-design :housing-design
                                         :mitsuho-design :hikari-design
                                         :tooling-design :compute-design
                                         :liquidity-design
                                         :priority-stack-offline
    ...
    (when-let [ps (:priority-stack-offline summary)]
      (when (true? (:ok ps))
        (pp/assert-no-public-scores!
         (dissoc ps :priority-order :mitsuho-care-first-before-rails
                 :hikari-care-first-before-rails :priority-stack :error :note :path))))
    (doseq [k [:care-design :housing-design :mitsuho-design :hikari-design
               :tooling-design :compute-design :liquidity-design]]
      (when-let [d (get summary k)]
        (pp/assert-no-public-scores!
         (dissoc d :note :multi-gen-facts :care-first-before-rails
    ...
    (when housing-recv-gated (pp/assert-no-public-scores! housing-recv-gated))
    (when housing-prod-gated (pp/assert-no-public-scores! housing-prod-gated))
    (when tooling-recv-gated (pp/assert-no-public-scores! tooling-recv-gated))
    (when tooling-prod-gated (pp/assert-no-public-scores! tooling-prod-gated))
    (when compute-recv-gated (pp/assert-no-public-scores! compute-recv-gated))
    (when compute-prod-gated (pp/assert-no-public-scores! compute-prod-gated))
    (when liquidity-recv-gated (pp/assert-no-public-scores! liquidity-recv-gated))
    out))

(defn run-with-itonami-seed
  "Offline path + itonami displacement public facts. Portable under bb and nbb."
  [opts]
  (let [itonami-seed (itonami/load-itonami-seed-file)
        fuchi-seed (edn/load-edn
                    #?(:clj (let [actor (or (System/getenv "FUCHI_ACTOR_DIR")
                                            (-> *file*
                                                java.io.File.
                                                .getParentFile .getParentFile
                                                .getCanonicalPath))]
                              (str (java.io.File. actor "data/seed-sustenance-graph.kotoba.edn")))
                       :cljs (let [path (js/require "node:path")
                                   actor (or (.-FUCHI_ACTOR_DIR (.-env js/process))
                                             (.resolve path (.dirname path *file*) ".."))]
                               (.join path actor "data" "seed-sustenance-graph.kotoba.edn"))))
