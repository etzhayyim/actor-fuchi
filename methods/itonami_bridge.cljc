...
(defn load-itonami-batch
  "Read itonami seed map or file path → vector of couple events."
  [seed-or-path]
  (let [m (cond
            (map? seed-or-path) seed-or-path
            (string? seed-or-path) (edn/load-edn seed-or-path)
            #?(:clj (instance? java.io.File seed-or-path) :cljs false)
            #?(:clj (edn/load-edn seed-or-path) :cljs nil)
            :else {})
        batch (or (get m ":events/batch")
                  (get m :events/batch)
                  [])]
    (mapv itonami->couple-event batch)))

(defn load-itonami-seed-file
  "Load data/itonami-displacement-events.edn from FUCHI_ACTOR_DIR / actor root."
  []
  (edn/load-edn (data-path "itonami-displacement-events.edn")))
