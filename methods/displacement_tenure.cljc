...
(defn tenure-climb-subject
  ...
  (let [person0 (person-from-subject subject)
        ...
        ;; Carry L0→L4 priority-(2) held-stress embed (itonami job-loss path) into tenure.
        out (cond-> out
              (:held-stress subject)
              (assoc :held-stress (:held-stress subject)
                     :held-stress-ladder-refused
                     (boolean (:held-stress-ladder-refused subject
                                                           (get-in subject [:held-stress
                                                                            :ladder-advance-refused])))
                     :held-stress-all-inkind-refused
                     (boolean (:held-stress-all-inkind-refused subject
                                                               (get-in subject [:held-stress
                                                                                :all-inkind-produce-rails-full-chain-refused])))
                     :l0-held-stress-carried true))]
    ...
    (pp/assert-no-public-scores!
     (select-keys out [:live :cash-usd-micros :score-surface :priority-stack
                       ...
                       :held-stress-ladder-refused :held-stress-all-inkind-refused
                       :l0-held-stress-carried]))
...
(defn run-tenure-for-package
  ...
  (if-not (= :offline-enrolled (:phase pkg))
    ...
    (let [ear (or (:earmark pkg) (couple/earmark-from-surplus event))
          ...
          hs-n (count (filter :held-stress tenured))
          hs-ladder-n (count (filter :held-stress-ladder-refused tenured))]
      (assoc pkg
             ...
             :tenure-held-stress-subjects hs-n
             :tenure-held-stress-ladder-refused-subjects hs-ladder-n
             :live false
             :cash-usd-micros 0
             :score-surface []
             :priority-stack PRIORITY-STACK))))
...
(defn run-batch-with-tenure
  ...
  (let [ev-by (into {} (map (fn [e] [(:cohort-id e) e]) events))
        ...
        all-tenured (mapcat :tenure-subjects packages)
        out (assoc batch
                   :packages packages
                   :tenure-target (ladder/normalize-stage target-stage)
                   :tenure-admissible-cohorts (count tenure-ok)
                   :tenure-subjects (count all-tenured)
                   ...
                   (count (filter :held-stress all-tenured))
                   :tenure-held-stress-ladder-refused-subjects
                   (count (filter :held-stress-ladder-refused all-tenured))
                   :live false
                   :cash-usd-micros 0
                   :score-surface []
                   :priority-stack PRIORITY-STACK)]
