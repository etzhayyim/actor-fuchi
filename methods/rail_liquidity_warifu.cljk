(ns fuchi.methods.rail-liquidity-warifu
  ...
  disclosure held → refuse. live=false. Portable .cljc (design EDN under bb + nbb)."
...
;; Portable design facts (no I/O). Mirrors data/rail-liquidity-warifu-design.edn residual slice.
(def DESIGN-PUBLIC-FACTS
  {:rail-kind RAIL-KIND
   ...
   :care-first-api-path "liquidity-residual-path"
   ...
   :cash-usd-micros 0
   :score-surface []
   :note "R1→gated DESIGN only; residual after multi-gen; fuchi never cash creditor"})

(defn design-public-facts
  "Facts-only projection of liquidity-warifu design (member-principal; cash≡0; loan never)."
  []
...
(defn load-design
  "Load data/rail-liquidity-warifu-design.edn. Portable under bb and nbb."
  []
  (edn/load-data "rail-liquidity-warifu-design.edn"))

(defn design-edn-invariants
  "Design EDN: cash≡0, member-principal residual, loan never."
  []
  (let [d (load-design)
    ...
    (when-not (zero? (long cash))
      (throw (ex-info "liquidity design cash≡0" out)))
