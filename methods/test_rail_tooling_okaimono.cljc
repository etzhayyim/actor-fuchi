...
(deftest test-design-public-facts-vocation
  (let [d (t/design-public-facts)]
    (is (= "tooling-okaimono" (:rail-kind d)))
    (is (= "vocation-recovery-path" (:care-first-api-path d)))
    (is (= ["care" "housing"] (:care-first-before-rails d)))
    (is (true? (:vocation-recovery d)))
    (is (false? (:fulfillment-executed d)))
    (is (false? (:live-produce d)))
    (is (false? (:live d)))
    (is (zero? (:cash-usd-micros d)))
    (is (= [] (:score-surface d)))
    (is (some #{"fulfillment-not-invoked"} (:multi-gen-facts d)))))
...
(deftest test-gated-plan
  ...
  (let [pkg (t/r1-dry-package {:alloc-id "a" :imputed-usd-micros-yr 1 :person (person fresh)})
    ...
    (is (= "vocation-recovery-path" (:care-first-api-path st)))
    (is (true? (:vocation-recovery st)))
